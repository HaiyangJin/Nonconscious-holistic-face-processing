...
```
Results of PCA show that the proportion of variances of four stimulus random effects and four participant random effects are less than 1%. 

```{r}
print(summary(glmm.zcp.acc), corr = FALSE)
```

These eight random effects (`Exp_Ali`, `Ali_D`, `Exp_Con`, `Con_Ali`, and  for Stimuli; `Exp_Con_Ali`, `Exp_Ali`, `Exp_Con`, `Ali_D`, and  for Participant) were removed from the first reduced model.
...
if (!file.exists(datafn.glmm.rdc1.acc)) {
    ...
    glmm.rdc1.acc <- glmer(isCorrect ~ Experiment * Congruency * Alignment + 
                               ...
                               (1 + Exp_D + Con_D + Exp_Con_Ali || Stimuli),
                           data = df.clean,
                           family = "binomial",
                           # verbose = TRUE,
                           control=glmerControl(optimizer = "nloptwrap",
