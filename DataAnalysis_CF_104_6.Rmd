...
### Reduced model
```{r reduced model for accuracy (glmm.rdc.acc)}
# reduce the random effects
glmm.rdc.acc <- glmer(isCorrect ~ Experiment * Congruency * Alignment + 
                          (1 + Exp_D + Con_D + Ali_D + Exp_Con + Exp_Ali + Con_Ali + Exp_Con_Ali || Participant) +
                          (1 + Exp_D + Con_D + Ali_D + Exp_Con + Exp_Ali + Con_Ali + Exp_Con_Ali || Stimuli),
                      data = df.clean,
                      family = "binomial",
...
anova(glmm.rdc.acc, glmm.zcp.acc)
```
...
anova(glmm.etd.acc, glmm.red.acc)
```
