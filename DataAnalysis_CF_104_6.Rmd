...
```
Results of PCA show that the proportion of variances of four stimulus random effects and four participant random effects are less than 1%. These eight random effects (`Exp_Ali`, `Exp_D`, `Con_Ali`, and `Ali_D` for Stimuli; `Exp_Con`, `Ali_D`, `Con_D`, and `Exp_Ali` for Participant) were removed from the first reduced model.
...
if (!file.exists(datafn.glmm.rdc1.acc)) {
    ...
    glmm.rdc1.acc <- glmer(isCorrect ~ Experiment * Congruency * Alignment + 
                               (1 + Exp_D + Con_Ali + Exp_Con_Ali || Participant) +
                               (1 + Con_D + Exp_Con + Exp_Con_Ali || Stimuli),
                           data = df.clean,
                           family = "binomial",
                           # verbose = TRUE,
                           control=glmerControl(optCtrl=list(maxfun=1e6))
...
```{r reduced model 2 for accuracy (glmm.rdc2.acc)}
# reduce the random effects
glmm.rdc2.acc <- glmer(isCorrect ~ Experiment * Congruency * Alignment + 
                          (1 + Exp_D + Con_Ali || Participant) +
                          (1 + Con_D + Exp_Con_Ali || Stimuli),
                      data = df.clean,
                      family = "binomial"
                      # verbose = TRUE
)

anova(glmm.rdc2.acc, glmm.rdc1.acc)
```


```{r rePCA for glmm.rdc2.acc}
# PCA for random effects of glmm.rdc2.acc
summary(rePCA(glmm.rdc2.acc))
```
...
# extend the reduced model with correlation parameters
...
anova(glmm.etd.acc, glmm.rdc2.acc)
```
