...
```{r setup and load the related libraries}
## set chunk options
...
This is a R markdown file to analyze the data for the <b>composite face task (complete design)</b> without <b>catch trials</b>. (Experiment number is 104_6) 
...
The raw data was loaded from the file named `r raw.file`. 

# Tidying up the data
```{r read the raw data file and set the name for output file}
...
```{r include = FALSE}
thisExp <- levels(raw.clean$Experiment)[1]
```
...
```{r include = FALSE}
thisExp <- levels(raw.clean$Experiment)[2]
```
