...
fn.raw <- "104_106_n-20_2017-05-04-1446.csv"
...
# bn.raw <- basename(fn.raw)
dir.raw <- dirname(fn.raw)

# set the filenames for R and SPSS data format
output.d.R <- file.path(dir.raw, paste0(output.name, "_d_R.csv"))
output.d.SPSS <- file.path(dir.raw, paste0(output.name, "_d_SPSS.csv"))
output.rt.R <- file.path(dir.raw, paste0(output.name, "_rt_R.csv"))
output.rt.SPSS <- file.path(dir.raw, paste0(output.name, "_rt_SPSS.csv"))

```

The raw data was loaded from the file named `r fn.raw`. 
...
# Logit Mixed Model
```{r create dummy coding for julia, include = FALSE}
# Create dummy coding
df.clean <- {
  df.clean %>%
    mutate(
      Experiment_D = ifelse(Experiment == "CFS", 0, ifelse(Experiment == "monocular", 1, NaN)),
      Congruency_D = ifelse(Congruency == "congruent", 0, ifelse(Congruency == "incongruent", 1, NaN)),
    ...
    filter(SameDifferent == "S")
}

write_csv(df.clean, "104_106_contrast.csv")
save(df.clean, file = "df_clean.RData")

path.nesi <- "nesi_fit"

```

## Full Logit Mixed Model
```{r logit mixed model -- full}
...
load(file.path(path.nesi, "S101_glmm_full.RData"))

print(summary(glmm.full), corr=FALSE)
```


```{r rePCA for the full model}
summary(rePCA(glmm.full))
```

## Reduced 1 Logit Mixed Model
```{r reduced 1 logistic mixed model}
...
load(file.path(path.nesi, "S101_glmm_reduced1.RData"))

print(summary(glmm.reduced1), corr=FALSE)
```


```{r rePCA for reduced1 model}
summary(rePCA(glmm.reduced1))
```


## Reduced 2 Logit Mixed Model
```{r reduced 2 logistic mixed model}
...
print(summary(glmm.reduced2), corr=FALSE)
```


```{r rePCA for reduced2 model}
summary(rePCA(glmm.reduced2))
```

## Reduced 3 Logit Mixed Model
```{r reduced 3 logistic mixed model}
glmm.reduced3 <- glmer(isCorrect ~ Experiment * Congruency * Alignment +
                         (0 + Experiment_D | Participant) +
                         (1 + Congruency_D + Alignment_D +
                            Congruency_Alignment | Stimuli),
                       data = df.clean,
                       family = "binomial",
                       # verbose = TRUE,
                       control=glmerControl(optCtrl=list(maxfun=1e6)))
...
print(summary(glmm.reduced3), corr=FALSE)
```


```{r rePCA for reduced3 model}
summary(rePCA(glmm.reduced3))
```


## Reduced 4 Logit Mixed Model
```{r reduced 4 logistic mixed model}
glmm.reduced4 <- glmer(isCorrect ~ Experiment * Congruency * Alignment +
                         (0 + Experiment_D | Participant) +
                         (1 + Congruency_D + Alignment_D | Stimuli),
                       data = df.clean,
                       family = "binomial",
                       # verbose = TRUE,
                       control=glmerControl(optCtrl=list(maxfun=1e6)))
...
print(summary(glmm.reduced4), corr=FALSE)
```


```{r rePCA for reduced4 model}
summary(rePCA(glmm.reduced4))
```


## Reduced 5 Logit Mixed Model
```{r reduced 5 logistic mixed model}
glmm.reduced5 <- glmer(isCorrect ~ Experiment * Congruency * Alignment +
                         (0 + Experiment_D | Participant) +
                         (1 + Congruency_D | Stimuli),
                       data = df.clean,
                       family = "binomial",
                       # verbose = TRUE,
                       control=glmerControl(optCtrl=list(maxfun=1e6)))
...
print(summary(glmm.reduced5), corr=FALSE)
```


```{r rePCA for reduced5 model}
summary(rePCA(glmm.reduced5))
```


```{r}
# Fixed effect reduced 1 logist mixed model

# glmm.freduced1 <- glmer(isCorrect ~ Experiment * Congruency + Experiment:Alignment + Congruency:Alignment + Experiment:Congruency:Alignment +
#                          (0 + Experiment_D | Participant) +
#                          (1 + Congruency_D | Stimuli),
#                        data = df.clean,
#                        family = "binomial",
...
#                        control=glmerControl(optCtrl=list(maxfun=1e6)))
#   
# print(summary(glmm.freduced1), corr=FALSE)
# anova(glmm.reduced5, glmm.freduced1)
```
...
```{r logit mixed model -- zero correlation parameter }
# build the full model
glmm.zcp <- glmer(isCorrect ~ Experiment * Congruency * Alignment + 
                     (1 + Experiment * Congruency * Alignment || Participant) +
                     (1 + Experiment * Congruency * Alignment || Stimuli),
                   data = df.clean,
                   family = "binomial",
                   verbose = TRUE,
                   control=glmerControl(optCtrl=list(maxfun=1e6)))

anova(glmm.full, glmm.zcp)
```
...
```{r save the theta for zcp model, include=FALSE}
theta_bk$zcp <- glmm.zcp@theta
```


```{r logit mixed model -- step}
# build the full model
glmm.zcp.step <- step(glmm.zcp, reduce.fixed = FALSE)

summary(glmm.zcp.step)
```

```{r}
glmm.reduced <- get_model(glmm.zcp.step)

anova(glmm.reduced, glmm.full)
```

```{r save the theta for reduced model, include=FALSE}
theta_bk$reduced <- glmm.reduced@theta

save(theta_bk, "theta_backup.RData")
```
