...
df.clean <- {
  df.clean %>%
    mutate(
      ...
      CF_Con = ifelse(Congruency == "congruent", 1, ifelse(Congruency == "incongruent", -1, NaN)),
      CF_Ali = ifelse(Alignment == "aligned", 1, ifelse(Alignment == "misaligned", -1, NaN)),
      CF_contrast = CF_Con * CF_Ali
    ) %>% 
    filter(SameDifferent == "S")
}

fn.df <- "S101_104_106_clean"
write_csv(df.clean, paste0(fn.df, ".csv"))
save(df.clean, file = paste0(fn.df, ".RData"))
...
glmm.full <- glmer(isCorrect ~ Experiment * CF_contrast +
                     (1 + Experiment * CF_contrast | Participant) +
                     (1 + Experiment * CF_contrast | Stimuli),
                   data = df.clean,
                   family = "binomial",
                   # verbose = TRUE,
