...
task.levels <- c("CFS", "monocular", "CatchOnly")
...
df.clean.E1 <- {
    df.raw.E1 %>%
        ...
        mutate(
            Task = case_when(  # rename the levels for Experiment
                Experiment == "104_CF_CFS" ~ task.levels[1],
                Experiment == "106_CF_Mono" ~ task.levels[2]
            ),
        ...
        group_by(Task, Participant, Congruency, Alignment, SameDifferent, isCorrect) %>% # divide into groups 
        mutate(
...
df.clean.cf.E1 <- {
    df.clean.E1 %>% 
        mutate(
            ...
            Task = recode_factor(Task, "CFS" = task.levels[1], "monocular" = task.levels[2]),
...
d.R.E1 <- { # get the hit and correct rejection rates
    df.clean.cf.E1 %>%
        group_by(Task, Participant, Congruency, Alignment, SameDifferent) %>%  # divide the data into serveral groups 
        ...
        select(Task, Participant, Congruency, Alignment, dprime, Count)
}
...
sum.d.R.E1 <- {
    d.R.E1 %>% 
        group_by(Task, Congruency, Alignment) %>% 
...
d.SPSS.E1 <- {
    d.R.E1 %>%
        ...
        mutate(Conditions = paste(Task, Congruency, Alignment, sep = ".")) %>% # combine all the levels together
...
d.RainPlot.E1 <- {
    ggplot(data = d.R.E1, aes(y = dprime, x = Alignment, fill = Congruency)) + 
        ...
        labs(title = "Sensitivity d' for Experiment 1", x = "Task", y = "Sensitivity", fill = "Congruency", color = "Congruency") +  # set the names for main, x and y axises and the legend
...
d.ColuPlot.E1 = {
    ggplot(data = sum.d.R.E1, aes(y = Mean, x = Alignment, color = Congruency, group = Congruency)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        ...
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        labs(title = "Sensitivity for Experiment 1", x = "Task", y = "Sensitivity", fill = "Congruency") +  # set the names for main, x and y axises
...
rt.R.E1 <- {
    df.clean.cf.E1 %>%
        ...
        group_by(Task, Participant, Congruency, Alignment) %>%  # divide the data into serveral groups 
...
rt.SPSS.E1 <- {
    rt.R.E1 %>%
        mutate(Conditions = paste(Task, Congruency, Alignment, sep = ".")) %>%
...
rt.ColuPlot.E1 = {
    ggplot(data = sum.rt.R.E1, aes(y = Mean, x = Alignment, color = Congruency, group = Congruency)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        ...
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        facet_grid(. ~ Task, switch = "both") +
        labs(title = "Correct Response Times for Experiment 1", x = "Task", y = "Response Times (ms)", fill = "Congruency") +  # set the names for main, x and y axises
        scale_fill_grey() +  # set the color for columns
...
df.clean.E2 <- {
    df.raw.E2 %>%
        ...
        mutate(
            Task = case_when(  # rename the levels for Experiment
                Experiment == "10802_CF_CFS_Catch_identity" ~ task.levels[1],
                Experiment == "10802_CF_CFS_mono_identity" ~ task.levels[2],
                Experiment == "10802_Mono_Catch_identity" ~ task.levels[3]
            ),
            TrialType = case_when(
        ...
        group_by(Task, Participant, TrialType, Congruency, Alignment, SameDifferent, isCorrect) %>% # divide into groups 
        mutate(
            RT.Z = scale(reactionTime), # calculate the Z value for response times within each group
            RT.Within3Z = if_else(RT.Z <= 3 & RT.Z >= -3, 1, if_else(RT.Z < -3 | RT.Z > 3, 0, NaN))
        ) %>%  # if the Z values are between -3 and 3, will be marked as 1. 
...
d.RainPlot.E2 <- {
    ggplot(data = d.R.E2, aes(y = dprime, x = Alignment, fill = Congruency)) + 
        ...
        # scale_color_brewer(palette = "Set1") +
        # scale_fill_brewer(palette = "Set1") +
        labs(title = "Sensitivity d' for Experiment 2", x = "Task", y = "Sensitivity", fill = "Congruency", color = "Congruency") +  # set the names for main, x and y axises and the legend
        # coord_cartesian(ylim = c(0, 6)) +  # set the limit for y axis
        # scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        # geom_hline(yintercept = 0, linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
        theme_bw() + # the backgroud color
...
d.ColuPlot.E2 = {
    ggplot(data = sum.d.R.E2, aes(y = Mean, x = Alignment, color = Congruency, group = Congruency)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        ...
        coord_cartesian(ylim = c(0, 4.5)) +  # set the limit for y axis
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        labs(title = "Sensitivity for Experiment 2", x = "Task", y = "Sensitivity", fill = "Congruency") +  # set the names for main, x and y axises
        scale_fill_grey() +  # set the color for columns
        geom_text(label = c("", "", "", "", "***", "", "", ""), color = "red", size = 6, nudge_y = 0.5, nudge_x = 0.5) + # add starts to the significant columns
        theme_bw() +
        plot_theme
...
rt.RainPlot.E2 <- {
    ggplot(data = cf.rt.R.E2, aes(y = RT, x = Alignment, fill = Congruency)) + 
        ...
        # scale_color_brewer(palette = "Set1") +
        # scale_fill_brewer(palette = "Set1") +
        labs(title = "Correct Response Times for Task 2", x = "Alignment", y = "Response Times (ms)", fill = "Congruency", color = "Congruency") +  # set the names for main, x and y axises and the legend
        # coord_cartesian(ylim = c(0, 6)) +  # set the limit for y axis
        # scale_y_continuous(expand = c(0, 0)) +  # remove the space between columns and x axis
        # geom_hline(yintercept = 0, linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
        theme_bw() + # the backgroud color
...
rt.ColuPlot.E2 = {
    ggplot(data = cf.sum.rt.R.E2, aes(y = Mean, x = Alignment, color = Congruency, group = Congruency)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        ...
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        facet_grid(. ~ Task, switch = "both") +
        labs(title = "Correct Response Times for Experiment 2", x = "Task", y = "Response Times (ms)", fill = "Congruency") +  # set the names for main, x and y axises
        scale_fill_grey() +  # set the color for columns
        geom_text(label = c("", "", "", "", "", "", "", ""), color = "red", size = 6, nudge_y = 30, nudge_x = 0.5) + # add starts to the significant columns
        theme_bw() +
        plot_theme
...
df.clean.catch.E2 <- {
    df.clean.E2 %>% 
        ...
        mutate(
            Participant = as.factor(Participant),
            Task = recode_factor(Task, "CFS" = task.levels[1], "monocular" = task.levels[2], "CatchOnly" = task.levels[3]),
            Alignment = recode_factor(Alignment, "A" = alignment.levels[1], "M" = alignment.levels[2])
        )
}
...
```{r accuracy for catch trials E2}
    ...
    df.clean.catch.E2 %>%
        filter(!(Participant %in% rmParticipant)) %>%  # remove participants
        group_by(Task, Participant, Alignment) %>% 
        ...
        mutate(Condition = factor(paste(Task, Alignment, sep = "."), levels = c("CFS.aligned", "CFS.misaligned", "monocular.aligned", "monocular.misaligned", "CatchOnly.aligned", "CatchOnly.misaligned"))) %>% 
...
knitr::kable(catch.sum.acc.R.E2, digits = 4)
    ...
    ggplot(data = catch.acc.R.E2, aes(y = Accuracy, x = Alignment)) + 
        ...
        # scale_color_brewer(palette = "Set1") +
        # scale_fill_brewer(palette = "Set1") +
        labs(title = "Accuracy for Catch Trials in Experiment 2", x = "Task", y = "Accuracy") +  # set the names for main, x and y axises and the legend
...
```{r Plot of accuracy for catch trials E2}
    ...
    ggplot(data = catch.sum.acc.R.E2, aes(y = Mean, x = Alignment)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        ...
        coord_cartesian(ylim = c(0, 1.05)) +  # set the limit for y axis
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        labs(title = "Accuracy of Catch Trials in Experiment 2", x = "Task", y = "Accuracy") +  # set the names for main, x and y axises
        scale_fill_grey() +  # set the color for columns
        geom_hline(yintercept = c(0.5, 1), linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
...
```{r calculate the correct response times of catch trials for R analysis E2}
    ...
    df.clean.catch.E2 %>%
        ...
        filter(RT.Within3Z == 1)  %>%  # only keep the RT within 3 Z
        # filter(isCorrect == 1)  %>%  # only keep the correct correct
        group_by(Task, Participant, Alignment) %>%  # divide the data into serveral groups 
        summarize(RT = mean(reactionTime) * 1000, Count = n()) %>% # get RT and count for each condition
        ungroup()
}
...
# RT for SPSS sytle analysis
        ...
        mutate(Conditions = factor(paste(Task, Alignment, sep = ".") , levels = c("CFS.aligned", "CFS.misaligned", "monocular.aligned", "monocular.misaligned", "CatchOnly.aligned", "CatchOnly.misaligned"))) %>%
        select(Participant, Conditions, RT) %>% 
        spread(Conditions, RT)
}
...
```{r Plot of response times for catch trials E2}
    ...
    ggplot(data = catch.sum.rt.R.E2, aes(y = Mean, x = Alignment)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        ...
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        labs(title = "Accuracy of Catch Trials in Experiment 2", x = "Task", y = "Response Times (ms)") +  # set the names for main, x and y axises
        scale_fill_grey() +  # set the color for columns
        # geom_hline(yintercept = c(0.5, 1), linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
        geom_text(label = c("", "", "", "", "", ""), color = "red", size = 6, nudge_y = 0.5, nudge_x = 0.5) + # add starts to the significant columns
        theme_bw() +
...
df.clean.E3 <- {
    df.raw.E3 %>%
        ...
        mutate(
            Task = case_when(  # rename the levels for Experiment
                Experiment == "111_CF_CFS_test" ~ task.levels[1],
                Experiment == "111_CF_monocular_test" ~ task.levels[2]
            ),
            TrialType = trialtype.levels[1],
            ExpCode = "E3",
            WithCatch = FALSE,
...
d.RainPlot.E3 <- {
    ggplot(data = d.R.E3, aes(y = dprime, x = Alignment, fill = Congruency)) + 
        ...
        # scale_color_brewer(palette = "Set1") +
        # scale_fill_brewer(palette = "Set1") +
        labs(title = "Sensitivity d' for Experiment 3", x = "Task", y = "Sensitivity", fill = "Congruency", color = "Congruency") +  # set the names for main, x and y axises and the legend
        # coord_cartesian(ylim = c(0, 6)) +  # set the limit for y axis
        # scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        # geom_hline(yintercept = 0, linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
        theme_bw() + # the backgroud color
...
d.ColuPlot.E3 = {
    ggplot(data = sum.d.R.E3, aes(y = Mean, x = Alignment, color = Congruency, group = Congruency)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        ...
        coord_cartesian(ylim = c(0, 4.5)) +  # set the limit for y axis
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        labs(title = "Sensitivity for Experiment 3", x = "Task", y = "Sensitivity", fill = "Congruency") +  # set the names for main, x and y axises
        scale_fill_grey() +  # set the color for columns
        geom_text(label = c("", "", "", "", "***", "", "", ""), color = "red", size = 6, nudge_y = 0.5, nudge_x = 0.5) + # add starts to the significant columns
        theme_bw() +
        plot_theme
...
rt.ColuPlot.E3 = {
    ggplot(data = cf.sum.rt.R.E3, aes(y = Mean, x = Alignment, color = Congruency, group = Congruency)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        ...
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        facet_grid(. ~ Task, switch = "both") +
        labs(title = "Correct Response Times for Experiment 3", x = "Task", y = "Response Times (ms)", fill = "Congruency") +  # set the names for main, x and y axises
        scale_fill_grey() +  # set the color for columns
        geom_text(label = c("", "", "", "", "", "", "", ""), color = "red", size = 6, nudge_y = 30, nudge_x = 0.5) + # add starts to the significant columns
        theme_bw() +
        plot_theme
...
df.clean.E4 <- {
    df.raw.E4 %>%
        ...
        mutate(
            Task = case_when(  # rename the levels for Experiment
                Experiment == "112_CF_CFS_catch_test" ~ task.levels[1],
                Experiment == "112_CF_monocular_catch_test" ~ task.levels[2],
                Experiment == "112_catch_test" ~ task.levels[3]
            ),
            TrialType = case_when(
...
# remove the participants whose dprime were less than 1 in at least one condition
rmParticipant1 <- unique(d.R.E4$Participant[(d.R.E4$dprime <= 0)]) # participants whose dprime is less than 0 
rmParticipant2 <- unique(d.R.E4$Participant[is.na(d.R.E4$dprime)]) # dprie is missing

rmLevels = levels(rmParticipant1)
rmParticipant <- factor(rmLevels[c(rmParticipant1, rmParticipant2)], levels = rmLevels)

d.R.E4 <- d.R.E4[!(d.R.E4$Participant %in% rmParticipant), ]
...
knitr::kable(sum.d.R.E4, digits = 4)
...
d.RainPlot.E4 <- {
    ggplot(data = d.R.E4, aes(y = dprime, x = Alignment, fill = Congruency)) + 
        ...
        geom_point(aes(y = dprime, color = Congruency), position = position_jitter(width = .15), size = .5, alpha = .8) +
        geom_boxplot(aes(y = dprime), width = .2, outlier.shape = NA, alpha = .7) +
        geom_point(data = sum.d.R.E4, aes(y = Mean, x = Alignment, color = Congruency), position = position_nudge(x = 0.3), size = 2.5) +
        geom_errorbar(data = sum.d.R.E4, aes(y = Mean, ymin = Lower, ymax = Upper), position = position_nudge(x = 0.3), width = 0) +
        facet_grid(. ~ Task, switch = "both") +
        # scale_colour_grey() + # start = .1, end = .6, color for the contour
        # scale_fill_grey() + # start = .3, end = .6, color for the fill
        # scale_color_brewer(palette = "Set1") +
        # scale_fill_brewer(palette = "Set1") +
        labs(title = "Sensitivity d' for Experiment 4", x = "Task", y = "Sensitivity", fill = "Congruency", color = "Congruency") +  # set the names for main, x and y axises and the legend
        # coord_cartesian(ylim = c(0, 6)) +  # set the limit for y axis
        # scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        # geom_hline(yintercept = 0, linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
        theme_bw() + # the backgroud color
...
d.ColuPlot.E4 = {
    ggplot(data = sum.d.R.E4, aes(y = Mean, x = Alignment, color = Congruency, group = Congruency)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        geom_point() +
        geom_line() +  # position = "dodge", alpha = .7
        geom_errorbar(mapping = aes(ymin = SE.lo, ymax = SE.hi), linetype = 1,  # set the error bar
                      show.legend = FALSE, width = 0.25, alpha = .5) + # ,                   position = position_dodge(width=0.9)
        ...
        coord_cartesian(ylim = c(0, 4.5)) +  # set the limit for y axis
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        labs(title = "Sensitivity for Experiment 4", x = "Task", y = "Sensitivity", fill = "Congruency") +  # set the names for main, x and y axises
        scale_fill_grey() +  # set the color for columns
        geom_text(label = c("", "", "", "", "***", "", "", ""), color = "red", size = 6, nudge_y = 0.5, nudge_x = 0.5) + # add starts to the significant columns
        theme_bw() +
        plot_theme
...
knitr::kable(cf.sum.rt.R.E4, digits = 4)
...
rt.RainPlot.E4 <- {
    ggplot(data = cf.rt.R.E4, aes(y = RT, x = Alignment, fill = Congruency)) + 
        ...
        geom_point(aes(y = RT, color = Congruency), position = position_jitter(width = .15), size = .5, alpha = .8) +
        geom_boxplot(aes(y = RT), width = .2, outlier.shape = NA, alpha = .7) +
        geom_point(data = cf.sum.rt.R.E4, aes(y = Mean, x = Alignment, color = Congruency), position = position_nudge(x = 0.3), size = 2.5) +
        geom_errorbar(data = cf.sum.rt.R.E4, aes(y = Mean, ymin = Lower, ymax = Upper), position = position_nudge(x = 0.3), width = 0) +
        facet_grid(. ~ Task, switch = "both") +
        # scale_colour_grey() + # start = .1, end = .6, color for the contour
        # scale_fill_grey() + # start = .3, end = .6, color for the fill
        # scale_color_brewer(palette = "Set1") +
...
rt.ColuPlot.E4 = {
    ggplot(data = cf.sum.rt.R.E4, aes(y = Mean, x = Alignment, color = Congruency, group = Congruency)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        geom_point() +
        geom_line() +  # position = "dodge", alpha = .7
        geom_errorbar(mapping = aes(ymin = SE.lo, ymax = SE.hi), linetype = 1,  # set the error bar
                      show.legend = FALSE, width = 0.25, alpha = .5) + # ,                   position = position_dodge(width=0.9)
        ...
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        facet_grid(. ~ Task, switch = "both") +
        labs(title = "Correct Response Times for Experiment 4", x = "Task", y = "Response Times (ms)", fill = "Congruency") +  # set the names for main, x and y axises
        scale_fill_grey() +  # set the color for columns
        geom_text(label = c("", "", "", "", "", "", "", ""), color = "red", size = 6, nudge_y = 30, nudge_x = 0.5) + # add starts to the significant columns
        theme_bw() +
        plot_theme
...
knitr::kable(catch.sum.acc.R.E4, digits = 4)
    ...
    ggplot(data = catch.acc.R.E4, aes(y = Accuracy, x = Alignment)) + 
        ...
        geom_point(position = position_jitter(width = .15, height = 0), size = .5, alpha = .8) +
        geom_boxplot(width = .2, outlier.shape = NA, alpha = .7) +
        geom_point(data = catch.sum.acc.R.E4, aes(y = Mean, x = Alignment), position = position_nudge(x = 0.3), size = 2.5) +
        geom_errorbar(data = catch.sum.acc.R.E4, aes(y = Mean, ymin = Lower, ymax = Upper), position = position_nudge(x = 0.3), width = 0) +
        facet_grid(. ~ Task, switch = "both") +
        # scale_colour_grey() + # start = .1, end = .6, color for the contour
        # scale_fill_grey() + # start = .3, end = .6, color for the fill
        # scale_color_brewer(palette = "Set1") +
        # scale_fill_brewer(palette = "Set1") +
        labs(title = "Accuracy for Catch Trials in Experiment 4", x = "Task", y = "Accuracy") +  # set the names for main, x and y axises and the legend
        coord_cartesian(ylim = c(0, 1.05)) +  # set the limit for y axis
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        geom_hline(yintercept = c(0.5, 1), linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
        theme_bw() + # the backgroud color
...
```{r Plot of accuracy for catch trials E4}
    ...
    ggplot(data = catch.sum.acc.R.E4, aes(y = Mean, x = Alignment)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        geom_col() +  # position = "dodge", alpha = .7
        geom_errorbar(mapping = aes(ymin = SE.lo, ymax = SE.hi), linetype = 1,  # set the error bar
                      show.legend = FALSE, width = 0.25, alpha = .5) + # ,                   position = position_dodge(width=0.9)
        facet_grid(. ~ Task, switch = "both") +
        coord_cartesian(ylim = c(0, 1.05)) +  # set the limit for y axis
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        labs(title = "Accuracy of Catch Trials in Experiment 4", x = "Task", y = "Accuracy") +  # set the names for main, x and y axises
        scale_fill_grey() +  # set the color for columns
        geom_hline(yintercept = c(0.5, 1), linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
        geom_text(label = c("", "", "", "", "***", "***"), color = "red", size = 6, nudge_y = 0.1, nudge_x = 0) + # add starts to the significant columns
        theme_bw() +
...
knitr::kable(catch.sum.rt.R.E4, digits = 4)
    ...
    ggplot(data = catch.rt.R.E4, aes(y = RT, x = Alignment)) + 
        ...
        geom_point(position = position_jitter(width = .15), size = .5, alpha = .8) +
        geom_boxplot(width = .2, outlier.shape = NA, alpha = .7) +
        geom_point(data = catch.sum.rt.R.E4, aes(y = Mean, x = Alignment), position = position_nudge(x = 0.3), size = 2.5) +
        geom_errorbar(data = catch.sum.rt.R.E4, aes(y = Mean, ymin = Lower, ymax = Upper), position = position_nudge(x = 0.3), width = 0) +
        facet_grid(. ~ Task, switch = "both") +
        # scale_colour_grey() + # start = .1, end = .6, color for the contour
        # scale_fill_grey() + # start = .3, end = .6, color for the fill
        # scale_color_brewer(palette = "Set1") +
...
```{r Plot of response times for catch trials E4}
    ...
    ggplot(data = catch.sum.rt.R.E4, aes(y = Mean, x = Alignment)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
        geom_col() +  # position = "dodge", alpha = .7
        geom_errorbar(mapping = aes(ymin = SE.lo, ymax = SE.hi), linetype = 1,  # set the error bar
                      show.legend = FALSE, width = 0.25, alpha = .5) + # ,                   position = position_dodge(width=0.9)
        facet_grid(. ~ Task, switch = "both") +
        coord_cartesian(ylim = c(0, 3200)) +  # set the limit for y axis
        scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
        labs(title = "Accuracy of Catch Trials in Experiment 4", x = "Task", y = "Response Times (ms)") +  # set the names for main, x and y axises
        scale_fill_grey() +  # set the color for columns
        # geom_hline(yintercept = c(0.5, 1), linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
        geom_text(label = c("", "", "", "", "", ""), color = "red", size = 6, nudge_y = 0.5, nudge_x = 0.5) + # add starts to the significant columns
        theme_bw() +
...
df.cf.all <- {
    df.all %>% 
        ...
        select(Task, Participant, Congruency, Alignment, FaceGroup, WithCatch, isCorrect, reactionTime, ExpCode, RT.Z)
}
...
df.cf.all <- {
    df.cf.all %>% 
        mutate(
            Task_D = if_else(Task == task.levels[1], 0, if_else(Task == task.levels[2], 1, NaN)),
            ...
            Task_Con = Task_D * Con_D, 
            Task_Ali = Task_D * Ali_D,
            Con_Ali = Con_D * Ali_D,
            
            Task_Con_Ali = Task_D * Con_D * Ali_D
        )
}
...
#### Maximum model
```{r}
file.max.acc <- file.path(folder.nesi, "P101_glmm_max_acc.RData")

if (!file.exists(file.max.acc)) {
    ...
    save(glmm.max.acc, file = file.max.acc)
} else {
    load(file.max.acc)
}
...
file.zcp.acc <- file.path(folder.output, "P101_glmm_zcp_acc.RData")

if (!file.exists(file.zcp.acc)) {
    ...
    save(glmm.zcp.acc, file = file.zcp.acc)
} else {
    load(file.zcp.acc)
}
...
### Reduced model 1
```{r}
glmm.rdc1.acc <- update(glmm.zcp.acc, 
                        formula = isCorrect ~ Task * Congruency * Alignment + ExpCode + 
                              (1 + Task_D + Task_Con + Task_Ali + Task_Con_Ali || Participant) + 
                              (1 + Ali_D + Task_Con || FaceGroup),
                        verbose = FALSE)

summary(glmm.rdc1.acc)
```
...
file.etd.acc <- file.path(folder.nesi, "P101_glmm_etd_acc.RData")

if (!file.exists(file.etd.acc)) {
    glmm.etd.acc <- update(glmm.rdc1.acc, 
                        formula = isCorrect ~ Task * Congruency * Alignment + ExpCode + 
                              (1 + Task_D + Task_Con + Task_Ali + Task_Con_Ali | Participant) + 
                              (1 + Ali_D + Task_Con | FaceGroup)
                       )

    save(glmm.etd.acc, file = file.etd.acc)
} else {
    load(file.etd.acc)
}

print(summary(glmm.etd.acc), corr = FALSE)
