...
d.R.E1 <- { # get the hit and correct rejection rates
    df.clean.cf.E1 %>%
        ...
        select(Viewing, Participant, Congruency, Alignment, dprime, Count)
}

# remove the participants whose dprime were less than 1 in at least one condition
rmParticipant1.E1 <- unique(d.R.E1$Participant[(d.R.E1$dprime <= 0)]) # participants whose dprime is less than 0 
rmParticipant2.E1 <- unique(d.R.E1$Participant[is.na(d.R.E1$dprime)]) # dprie is missing

rmLevels = levels(rmParticipant1.E1)
rmParticipant.E1 <- factor(rmLevels[c(rmParticipant1.E1, rmParticipant2.E1)], levels = rmLevels)

d.R.E1 <- d.R.E1[!(d.R.E1$Participant %in% rmParticipant.E1), ]
...
# remove the participants whose dprime were less than 1 in at least one condition
rmParticipant1.E2 <- unique(d.R.E2$Participant[(d.R.E2$dprime <= 0)]) # participants whose dprime is less than 0 
rmParticipant2.E2 <- unique(d.R.E2$Participant[is.na(d.R.E2$dprime)]) # dprie is missing

rmLevels = levels(rmParticipant1.E2)
rmParticipant.E2 <- factor(rmLevels[c(rmParticipant1.E2, rmParticipant2.E2)], levels = rmLevels)

d.R.E2 <- d.R.E2[!(d.R.E2$Participant %in% rmParticipant.E2), ]
...
```
Participant `r rmParticipant1.E2` were removed due to at least one of their dprime was not larger than 0.

Participant `r rmParticipant2.E2` were removed due to at least one of their dprime was missing.
...
cf.rt.R.E2 <- {
    df.clean.cf.E2 %>%
        filter(!(Participant %in% rmParticipant.E2)) %>%  # remove participants
        filter(RT.Within3Z == 1)  %>%  # only keep the RT within 3 Z
        filter(isCorrect == 1)  %>%  # only keep the correct correct
        group_by(Viewing, Participant, Congruency, Alignment) %>%  # divide the data into serveral groups 
...
# remove the participants whose dprime were less than 1 in at least one condition
rmParticipant1.E3 <- unique(d.R.E3$Participant[(d.R.E3$dprime <= 0)]) # participants whose dprime is less than 0 
rmParticipant2.E3 <- unique(d.R.E3$Participant[is.na(d.R.E3$dprime)]) # dprie is missing

rmLevels = levels(rmParticipant1.E3)
rmParticipant.E3 <- factor(rmLevels[c(rmParticipant1.E3, rmParticipant2.E3)], levels = rmLevels)

d.R.E3 <- d.R.E3[!(d.R.E3$Participant %in% rmParticipant.E3), ]
...
Participant `r rmParticipant1.E3` were removed due to at least one of their dprime was not larger than 0.
...
cf.rt.R.E3 <- {
    df.clean.cf.E3 %>%
        filter(!(Participant %in% rmParticipant.E3)) %>%  # remove participants
        filter(RT.Within3Z == 1)  %>%  # only keep the RT within 3 Z
        filter(isCorrect == 1)  %>%  # only keep the correct correct
        group_by(Viewing, Participant, Congruency, Alignment) %>%  # divide the data into serveral groups 
        summarize(RT = mean(reactionTime) * 1000, Count = n()) %>% # get RT and count for each condition
...
# remove the participants whose dprime were less than 1 in at least one condition
rmParticipant1.E4 <- unique(d.R.E4$Participant[(d.R.E4$dprime <= 0)]) # participants whose dprime is less than 0 
rmParticipant2.E4 <- unique(d.R.E4$Participant[is.na(d.R.E4$dprime)]) # dprie is missing

rmLevels = levels(rmParticipant1.E4)
rmParticipant.E4 <- factor(rmLevels[c(rmParticipant1.E4, rmParticipant2.E4)], levels = rmLevels)

d.R.E4 <- d.R.E4[!(d.R.E4$Participant %in% rmParticipant.E4), ]
...
```
Participant `r rmParticipant1.E4` were removed due to at least one of their dprime was not larger than 0.

Participant `r rmParticipant2.E4` were removed due to at least one of their dprime was missing.
...
cf.rt.R.E4 <- {
    df.clean.cf.E4 %>%
        filter(!(Participant %in% rmParticipant.E4)) %>%  # remove participants
        filter(RT.Within3Z == 1)  %>%  # only keep the RT within 3 Z
        filter(isCorrect == 1)  %>%  # only keep the correct correct
        group_by(Viewing, Participant, Congruency, Alignment) %>%  # divide the data into serveral groups 
        summarize(RT = mean(reactionTime) * 1000, Count = n()) %>% # get RT and count for each condition
...
rmParticipant <- c(as.character(rmParticipant.E1), as.character(rmParticipant.E2), as.character(rmParticipant.E3), as.character(rmParticipant.E4))
...
#### Model selections
##### Maximal model
```{r maximal model for accuracy}
# maximal model for accuracy
...
```{r maximal model 2 for accuracy}
# maximal model 2 for accuarcy (starting with the parameters from maximal model)
file.max2.acc <- file.path(folder.nesi, "P101_glmm_max2_acc.RData")

if (!file.exists(file.max2.acc)) {
    pars.max.acc <- get_pars(glmm.max.acc)
    glmm.max2.acc <- update(glmm.max.acc, start = pars.max.acc)

    save(glmm.max2.acc, file = file.max2.acc)
} else {
    load(file.max2.acc)
}

print(summary(glmm.max2.acc), corr = FALSE)

```


##### Zero-correlation-parameter model
```{r zcp model for accuarcy}
# zero-correlation-parameter model for accuracy
file.zcp.acc <- file.path(folder.nesi, "P101_glmm_zcp_acc.RData")
...
```{r zcp model 2 for accuracy}
# zero-correlation-parameter model 2 for accuracy
file.zcp2.acc <- file.path(folder.nesi, "P101_glmm_zcp2_acc.RData")

if (!file.exists(file.zcp2.acc)) {
    pars.zcp.acc <- get_pars(glmm.zcp.acc)
    glmm.zcp2.acc <- update(glmm.zcp.acc, start = pars.zcp.acc)

    save(glmm.zcp2.acc, file = file.zcp2.acc)
} else {
    load(file.zcp2.acc)
}

print(summary(glmm.zcp2.acc), corr = FALSE)

```


```{r PCA for zcp model}
summary(rePCA(glmm.zcp2.acc))

```


##### Reduced model
```{r reduced model for accuracy}
# reduced model for accuracy
file.rdc.acc <- file.path(folder.nesi, "P101_glmm_rdc_acc.RData")

if (!file.exists(file.rdc.acc)) {
    ...
    save(glmm.rdc.acc, file = file.rdc.acc)
} else {
    load(file.rdc.acc)
}

print(summary(glmm.rdc.acc), corr = FALSE)

```

```{r reduced model 2 for accuarcy}
# reduced model 2 for accuarcy (start from parameters of glmm.rdc.acc)
file.rdc2.acc <- file.path(folder.nesi, "P101_glmm_rdc2_acc.RData")

if (!file.exists(file.rdc2.acc)) {
    pars.rdc.acc <- get_pars(glmm.rdc.acc)
    
    glmm.rdc2.acc <- update(glmm.rdc.acc, start = pars.rdc.acc)
    
    save(glmm.rdc2.acc, file = file.rdc2.acc)
} else {
    load(file.rdc2.acc)
}

print(summary(glmm.rdc2.acc), corr = FALSE)

```


```{r}
anova(glmm.zcp2.acc, glmm.rdc2.acc)
```

##### Extended model
```{r extended model for accuarcy}
# extended model for accuarcy
...
```{r extended model 2 for accuarcy}
# extended model 2 for accuarcy (start from parameters of glmm.etd.acc)
file.etd2.acc <- file.path(folder.nesi, "P101_glmm_etd2_acc.RData")

if (!file.exists(file.etd2.acc)) {
    pars.etd.acc <- get_pars(glmm.etd.acc)
    
    glmm.etd2.acc <- update(glmm.etd.acc, start = pars.etd.acc)
    
    save(glmm.etd2.acc, file = file.etd2.acc)
} else {
    load(file.etd2.acc)
}

print(summary(glmm.etd2.acc), corr = FALSE)

```

```{r}
summary(rePCA(glmm.etd2.acc))
```

```{r}
# extended model for accuarcy
# file.etd3.acc <- file.path(folder.nesi, "P101_glmm_etd3_acc.RData")
# 
# if (!file.exists(file.etd3.acc)) {
# 
# 
#     save(glmm.etd3.acc, file = file.etd3.acc)
# } else {
#     load(file.etd3.acc)
# }
glmm.etd3.acc <- update(glmm.etd.acc,
                        formula = isCorrect ~ Viewing * Congruency * Alignment + ExpCode +
                            (1 + View_D + View_Con + View_Ali | Participant) + 
                            (1 + View_Con | FaceGroup),
                        verbose = FALSE)
print(summary(glmm.etd3.acc), corr = FALSE)

```

```{r}
pars.etd3.acc <- get_pars(glmm.etd3.acc)

glmm.etd4.acc <- update(glmm.etd3.acc, start = pars.etd3.acc)

summary(glmm.etd4.acc)
```



```{r}
anova(glmm.rdc2.acc, glmm.etd4.acc)
```

##### Optimal model
```{r}
glmm.opt.acc <- glmm.rdc2.acc
summary(glmm.opt.acc)
...
```{r}
emm.acc <- emmeans(glmm.opt.acc, ~ Alignment | c(Congruency, Viewing))

data.frame(summary(emm.acc)) %>% 
...
#### Plots 
```{r}
emmip(glmm.opt.acc, Congruency ~ Alignment | Viewing, CIs = TRUE)
...
#### Contrasts
```{r}
contrast(emm.acc, "pairwise", simple = "each", combine = TRUE )
```

```{r}
contrast(emm.acc, interaction = "pairwise", by = "Viewing")
```
...
file.max.rt <- file.path(folder.nesi, "P101_glmm_max_rt.RData")
