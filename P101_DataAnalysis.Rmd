...
d.Count.E1 <- {
    df.clean.cf.E1 %>%
        ...
        summarize(Count = n()) %>% 
        mutate(Conditions = paste(Viewing, Congruency, Alignment, SameDifferent, sep = ".")) %>% 
        ungroup() %>% 
...
```{r check the number of remaining trials E2}
# number of trials in each condition
d.Count.E2 <- {
    df.clean.cf.E2 %>%
        group_by(Viewing, Participant, Congruency, Alignment, SameDifferent) %>%  # divide the data into serveral groups 
...
rm.count.E2 <- {
    d.Count.E2 %>% 
    filter(Count < max(d.Count.E2$Count) * rm.Count.rate) %$%
    unique(Participant)
    }
...
```{r check the number of remaining trials E4}
# number of trials in each condition
d.Count.E4 <- {
    df.clean.cf.E4 %>%
        group_by(Viewing, Participant, Congruency, Alignment, SameDifferent) %>%  # divide the data into serveral groups 
...
rm.count.E4 <- {
    d.Count.E4 %>% 
    filter(Count < max(d.Count.E4$Count) * rm.Count.rate) %$%
    unique(Participant)
    }
