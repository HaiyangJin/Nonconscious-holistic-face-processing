...
if (saveCSV) {
    ...
    output.catch.acc.R <- paste(raw.dir, "/", output.name, "_catch_acc_R.csv", sep = "")
    output.catch.acc.SPSS <- paste(raw.dir, "/", output.name, "_catch_acc_SPSS.csv", sep = "")
    output.catch.rt.R <- paste(raw.dir, "/", output.name, "_catch_rt_R.csv", sep = "")
    output.catch.rt.SPSS <- paste(raw.dir, "/", output.name, "_catch_rt_SPSS.csv", sep = "")
}

```

The raw data for Experiment 1 was loaded from the file named `r basename(fn.raw.E2)` in the *`r dirname(fn.raw.E2)`/* folder. 
...
# calculate the d'
cf.d.R.E2 <- { # get the hit and correct rejection rates
  df.clean.E2 %>%
...
rmParticipant <- unique(cf.d.R.E2$Participant[(cf.d.R.E2$dprime <= 0) | is.na(cf.d.R.E2$dprime)]) # participants whose dprime is less than 0 or missing

cf.d.R.E2 <- cf.d.R.E2[!(cf.d.R.E2$Participant %in% rmParticipant), ]
...
# save the d' for R analysis
if (saveCSV) {
  write.csv(cf.d.R.E2, output.d.R, row.names = FALSE)
  write.csv(cf.d.SPSS.E2, output.d.SPSS, row.names = FALSE)
}
```
...
# save the RT for R analysis
if (saveCSV) {
  write.csv(rt.R.E2, output.rt.R, row.names = FALSE)
  write.csv(rt.SPSS.E2, output.rt.SPSS, row.names = FALSE)
}
...
knitr::kable(sum.rt.R.E2, digits = 4)

rt.RainPlot.E2 <- {
  ggplot(data = rt.R.E2, aes(y = RT, x = Alignment, fill = Congruency)) + 
    geom_flat_violin(position = position_nudge(x = .2, y = 0), alpha = .7) +
    geom_point(aes(y = RT, color = Congruency), position = position_jitter(width = .15), size = .5, alpha = .8) +
    geom_boxplot(aes(y = RT), width = .2, outlier.shape = NA, alpha = .7) +
    geom_point(data = sum.rt.R.E2, aes(y = Mean, x = Alignment, color = Congruency), position = position_nudge(x = 0.3), size = 2.5) +
    geom_errorbar(data = sum.rt.R.E2, aes(y = Mean, ymin = Lower, ymax = Upper), position = position_nudge(x = 0.3), width = 0) +
    facet_grid(. ~ Experiment, switch = "both") +
    # scale_colour_grey() + # start = .1, end = .6, color for the contour
    # scale_fill_grey() + # start = .3, end = .6, color for the fill
    # scale_color_brewer(palette = "Set1") +
...
rt.ColuPlot.E2 = {
  ggplot(data = sum.rt.R.E2, aes(y = Mean, x = Alignment, color = Congruency, group = Congruency)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    geom_point() +
    geom_line() +  # position = "dodge", alpha = .7
    geom_errorbar(mapping = aes(ymin = SE.lo, ymax = SE.hi), linetype = 1,  # set the error bar
                  show.legend = FALSE, width = 0.25, alpha = .5) + # ,                   position = position_dodge(width=0.9)
