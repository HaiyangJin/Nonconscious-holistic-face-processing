...
df.cf.all <- {
  df.all %>% 
    ...
    select(Viewing, Participant, Congruency, Alignment, FaceGroup, WithCatch, isCorrect, reactionTime, ExpCode, Stimuli, RT.Z)
}
...
# }
# 
# allFit.etd.resp <- summary(glmm.etd.resp.allFit)
# 
# allFit.etd.resp
...
```{r}
lmm.zcp1.d <- update(lmm.max.d,
                    formula = dprime ~ Viewing * Congruency * Alignment + ExpCode +
                    (1 + View_D + Con_D + View_Con + Con_Ali || Participant))

print(summary(lmm.zcp1.d), corr = FALSE)
```
...
```{r}
contrast(emm.d.all, interaction = "pairwise", by = "Viewing")
```
...
```{r}
summary(rePCA(lmm.etd.rt))
```
One of the random effects did not contribute to the variances of the whole model. The variance of `Ali_D` was the smallest and its corresponding fixed effect was not significant. Therefore, the random effect of `Ali_D` was removed from the model.

```{r}
lmm.etd1.rt <- update(lmm.etd.rt,
                      formula = reactionTime ~ Viewing * Congruency * Alignment + ExpCode +
                        (1 + View_D + Con_D | Participant) +
                        (1 | Stimuli),
                      verbose = FALSE)
...
```{r}
anova(lmm.etd1.rt, lmm.rdc.rt)
```

##### The optimal model
```{r}
lmm.opt.rt <- update(lmm.etd1.rt,
                     REML = TRUE,
                     verbose = FALSE)
