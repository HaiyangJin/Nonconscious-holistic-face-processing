...
df.clean.E1 <- {
    df.raw.E1 %>%
        ...
        mutate(
            ...
            Experiment = recode_factor(Experiment, "104_CF_CFS" = task.levels[1], "106_CF_Mono" = task.levels[2]),
...
df.clean.E2 <- {
  df.raw.E2 %>%
    ...
    mutate(
      Participant = as.factor(Participant),
      Experiment = recode_factor(Experiment, "10802_CF_CFS_Catch_identity" = task.levels[1], "10802_CF_CFS_mono_identity" = task.levels[2], "10802_Mono_Catch_identity" = task.levels[3]),
      TrialType = recode_factor(Condition, "Complete" = trialtype.levels[1], "CatchOnly" = trialtype.levels[2]),
      Congruency = recode_factor(Congruency, "C" = congruency.levels[1], "I" = congruency.levels[2]), # rename the levels for Congruency
      Alignment = recode_factor(Alignment, "A" = alignment.levels[1], "M" = alignment.levels[2]),
      SameDifferent = recode_factor(SameDifferent, "S" = sameDifferent.levels[1], "D" = sameDifferent.levels[2])
      ) %>% # rename the levels for Alignment
    group_by(Experiment, Participant, TrialType, Congruency, Alignment, SameDifferent, isCorrect) %>% # divide into groups 
    mutate(
      RT.Z = scale(reactionTime), # calculate the Z value for response times within each group
      RT.Within3Z = ifelse(RT.Z <= 3 & RT.Z >= -3, 1, ifelse(RT.Z < -3 | RT.Z > 3, 0, NaN))
      ) %>%  # if the Z values are between -3 and 3, will be marked as 1. 
...
df.clean.E3 <- {
  df.raw.E3 %>%
    ...
    mutate(
      Participant = as.factor(Participant),
      Experiment = recode_factor(Experiment, "111_CF_CFS_test" = task.levels[1], "111_CF_monocular_test" = task.levels[2]),
      Congruency = recode_factor(Congruency, "C" = congruency.levels[1], "I" = congruency.levels[2]), # rename the levels for Congruency
      Alignment = recode_factor(Alignment, "A" = alignment.levels[1], "M" = alignment.levels[2]),
      # SameDifferent = recode_factor(SameDifferent, "S" = sameDifferent.levels[1], "D" = sameDifferent.levels[2]),
      TrialType = trialtype.levels[1],
...
The raw data for Experiment 3 was loaded from the file named `r basename(fn.raw.E4)` in the *`r dirname(fn.raw.E4)`/* folder. 
...
df.clean.E4 <- {
  df.raw.E4 %>%
    ...
    mutate(
      Participant = as.factor(Participant),
      Experiment = recode_factor(Experiment, "112_CF_CFS_catch_test" = task.levels[1], "112_CF_monocular_catch_test" = task.levels[2], "112_catch_test" = task.levels[3]),
      TrialType = recode_factor(TrialType, "CF" = trialtype.levels[1], "Catch" = trialtype.levels[2]),
      Congruency = ifelse(TrialType == "Catch", NA, Congruency),  # save the Congruency for catch trials as NA
      Congruency = recode_factor(Congruency, "C" = "congruent", "I" = "incongruent"),  # rename the levels for Congruency
      Alignment = recode_factor(Alignment, "A" = "aligned", "M" = "misaligned"),
      SameDifferent = recode_factor(SameDifferent, "S" = sameDifferent.levels[1], "D" = sameDifferent.levels[2])
      ) %>% # rename the levels for Alignment
    group_by(Experiment, Participant, TrialType, Congruency, Alignment, SameDifferent, isCorrect) %>% # divide into groups 
    mutate(
...
d.RainPlot.E4 <- {
  ggplot(data = d.R.E4, aes(y = dprime, x = Alignment, fill = Congruency)) + 
    ...
    # scale_color_brewer(palette = "Set1") +
    # scale_fill_brewer(palette = "Set1") +
    labs(title = "Sensitivity d' for Experiment 4", x = "Experiment", y = "Sensitivity", fill = "Congruency", color = "Congruency") +  # set the names for main, x and y axises and the legend
    # coord_cartesian(ylim = c(0, 6)) +  # set the limit for y axis
    # scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    # geom_hline(yintercept = 0, linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
    theme_bw() + # the backgroud color
...
d.ColuPlot.E4 = {
  ggplot(data = sum.d.R.E4, aes(y = Mean, x = Alignment, color = Congruency, group = Congruency)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    ...
    coord_cartesian(ylim = c(0, 4.5)) +  # set the limit for y axis
    scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    labs(title = "Sensitivity for Experiment 4", x = "Experiment", y = "Sensitivity", fill = "Congruency") +  # set the names for main, x and y axises
    scale_fill_grey() +  # set the color for columns
    geom_text(label = c("", "", "", "", "***", "", "", ""), color = "red", size = 6, nudge_y = 0.5, nudge_x = 0.5) + # add starts to the significant columns
    theme_bw() +
    plot_theme
...
rt.ColuPlot.E4 = {
  ggplot(data = cf.sum.rt.R.E4, aes(y = Mean, x = Alignment, color = Congruency, group = Congruency)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    ...
    scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    facet_grid(. ~ Experiment, switch = "both") +
    labs(title = "Correct Response Times for Experiment 4", x = "Experiment", y = "Response Times (ms)", fill = "Congruency") +  # set the names for main, x and y axises
    scale_fill_grey() +  # set the color for columns
    geom_text(label = c("", "", "", "", "", "", "", ""), color = "red", size = 6, nudge_y = 30, nudge_x = 0.5) + # add starts to the significant columns
    theme_bw() +
    plot_theme
...
knitr::kable(catch.sum.acc.R.E4, digits = 4)
  ...
  ggplot(data = catch.acc.R.E4, aes(y = Accuracy, x = Alignment)) + 
    ...
    # scale_color_brewer(palette = "Set1") +
    # scale_fill_brewer(palette = "Set1") +
    labs(title = "Accuracy for Catch Trials in Experiment 4", x = "Experiment", y = "Accuracy") +  # set the names for main, x and y axises and the legend
    coord_cartesian(ylim = c(0, 1.05)) +  # set the limit for y axis
    scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    geom_hline(yintercept = c(0.5, 1), linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
    theme_bw() + # the backgroud color
...
```{r Plot of accuracy for catch trials E4}
  ...
  ggplot(data = catch.sum.acc.R.E4, aes(y = Mean, x = Alignment)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    ...
    coord_cartesian(ylim = c(0, 1.05)) +  # set the limit for y axis
    scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    labs(title = "Accuracy of Catch Trials in Experiment 4", x = "Experiment", y = "Accuracy") +  # set the names for main, x and y axises
    scale_fill_grey() +  # set the color for columns
    geom_hline(yintercept = c(0.5, 1), linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
    geom_text(label = c("", "", "", "", "***", "***"), color = "red", size = 6, nudge_y = 0.1, nudge_x = 0) + # add starts to the significant columns
    theme_bw() +
...
```{r Plot of response times for catch trials}
  ...
  ggplot(data = catch.sum.rt.R.E4, aes(y = Mean, x = Alignment)) +  # set the data, varialbes for x and y axises, and the variable for dividing data
    ...
    coord_cartesian(ylim = c(0, 3200)) +  # set the limit for y axis
    scale_y_continuous(expand= c(0, 0)) +  # remove the space between columns and x axis
    labs(title = "Accuracy of Catch Trials in Experiment 4", x = "Experiment", y = "Response Times (ms)") +  # set the names for main, x and y axises
    scale_fill_grey() +  # set the color for columns
    # geom_hline(yintercept = c(0.5, 1), linetype = 5, alpha = 0.5) + # add the line for 0.5 and 1 (y)
    geom_text(label = c("", "", "", "", "", ""), color = "red", size = 6, nudge_y = 0.5, nudge_x = 0.5) + # add starts to the significant columns
    theme_bw() +
